...
func RegisterPeriod(rg *gin.RouterGroup) {
	g := rg.Group("/period")
	g.GET("/holding/:symbol", getHoldingBySymbol)
}

// getHoldingBySymbol 查詢特定 symbol 的集保持股比例 (stock_holder_percent)。
...
func getHoldingBySymbol(c *gin.Context) {
	...
	if limit, ok := optionalLimit(c); ok {
		...
		query = `SELECT symbol, trade_date, name, major_ratio, retail_ratio FROM (
				SELECT symbol, trade_date, name, major_ratio, retail_ratio
				FROM stock_holder_percent
				WHERE symbol = $1
				ORDER BY trade_date DESC LIMIT $2
			) t ORDER BY trade_date ASC`
		args = append(args, limit)
	} else {
		query = `SELECT symbol, trade_date, name, major_ratio, retail_ratio
			FROM stock_holder_percent
			WHERE symbol = $1
